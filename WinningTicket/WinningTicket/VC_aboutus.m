...
-(void)viewDidLayoutSubviews
{
    ...
    _scroll_contents.contentSize = CGSizeMake(_scroll_contents.frame.size.width, _lbl_contents.frame.size.height + 50);
}
...
-(void) setup_VIEW
{
    ...
    if ([self.lbl_contents respondsToSelector:@selector(setAttributedText:)]) {
        ...
        NSRange cmp = [text rangeOfString:str_title];// * Notice that usage of rangeOfString in this case may cause some bugs - I use it here only for demonstration
        
        if ( UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPad )
        {
            [attributedText setAttributes:@{NSFontAttributeName:[UIFont fontWithName:@"HelveticaNeue-Bold" size:28.0]}
                                    range:cmp];
        }
        else
        {
