...
@interface NEWCurrencyTextFieldDelegate : NSObject <UITextFieldDelegate>
@property (weak, nonatomic) id<UITextFieldDelegate> delegate;
@end

@implementation NEWCurrencyTextField
{
    NEWCurrencyTextFieldDelegate* _currencyTextFieldDelegate;
...
- (id) initWithCoder: (NSCoder *) aDecoder
{
    ...
    if ( self )
    {
        [self NEWCurrencyTextField_commonInit];
    }
    return self;
}
...
- (void) NEWCurrencyTextField_commonInit
{
...
//    _currencyNumberFormatter.locale = [NSLocale currentLocale];
    ...
    _currencyTextFieldDelegate = [NEWCurrencyTextFieldDelegate new];
...
@implementation NEWCurrencyTextFieldDelegate
...
- (BOOL) textField: (NEWCurrencyTextField *) textField shouldChangeCharactersInRange: (NSRange) range replacementString: (NSString *) string
{
