...
{
    _floatingLabelActiveTextColor = [UIColor blackColor];
    if (self.isEditing)
...
- (void)setupDefaultColorStates {
    ...
    self.floatingLabelInactiveTextColor = self.floatingLabelInactiveTextColor ?: [UIColor blackColor];
...
- (void)drawRect:(CGRect)aRect
{
    ...
    if (self.shouldDrawPlaceholder) {
        ...
        CGRect placeholderFrame = CGRectMake(aRect.origin.x + 5, floorf(self.frame.size.height / 2.f), self.frame.size.width - 10, self.frame.size.height);
...
- (void)adjustFramesForNewPlaceholder
{
    ...
    CGFloat offset = 5.0f;//ceil(self.floatingLabel.font.lineHeight);
    ...
    self.offsetTextFieldFrame = CGRectMake(self.originalTextFieldFrame.origin.x, self.originalTextFieldFrame.origin.y,
...
{
    return [super editingRectForBounds:UIEdgeInsetsInsetRect(bounds, UIEdgeInsetsMake(3.f, 0.f, 0.f, 5.f))];
}
